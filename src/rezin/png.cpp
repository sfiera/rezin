...
void png_error(png_struct* png, png_const_charp error_msg) {
    throw std::runtime_error(pn::format("png error: {0}", error_msg).c_str());
}

void png_write_data(png_struct* png, png_byte* data, png_size_t length) {
    pn::file_view* out = reinterpret_cast<pn::file_view*>(png_get_io_ptr(png));
    out->write(pn::data_view{data, static_cast<int>(length)}).check();
}
...
PngWriter::PngWriter(pn::file_view out, int32_t width, int32_t height)
